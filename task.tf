...
connection {
    type     = "ssh"
    user     = "ec2-user"
    private_key = tls_private_key.tls_key.private_key_pem//file("C:/Users/hp/Desktop/code_terraform/task/taskey.pem")
...
resource "aws_ebs_volume" "myvoltask"{
...
resource "aws_volume_attachment" "ebs_att" {
  device_name = "/dev/sdd"
  volume_id   = "${aws_ebs_volume.myvoltask.id}"
  instance_id = "${aws_instance.task1.id}"
  force_detach = true
  depends_on = [
    aws_ebs_volume.myvoltask,
     aws_instance.task1
  ]
}
...
connection {
    ...
    inline = [
      ...
      "sudo git clone https://github.com/arushi09-hub/multicloud.git /var/www/html"
    ]
  }
}
...
resource "aws_s3_bucket_object" "s3obj" {
  bucket = "task1"
  key = "download.png"
  source = "C:/Users/hp/Desktop/code_terraform/task/download.png"
  acl    = "public-read"
